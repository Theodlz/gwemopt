"""
Core scheduler functions for GWEMOpt.
"""
...
def get_altaz_tiles(
    ...
    """
    Convert a grid of RA, Dec points to Alt, Az at a given observatory and time.

    :param ras: list of RA values (degrees)
    :param decs: list of Dec values (degrees)
    :param observatory: astropy.coordinates.EarthLocation
...
def find_tile(
    ...
    """
    Find the next tile to observe.

    :param exposureids_tile: {expo id}-> list of the tiles available for observation
    :param exposureids: list of tile ids for every exposure it is allocated to observe
    :param probs: list of probabilities for every exposure it is allocated to observe
    :param idxs: list of indices to check
    :param exptimecheckkeys: list of keys to check for exposure time
    :param current_ra: current RA (degrees)
    :param current_dec: current Dec (degrees)
    :param slew_rate: slew rate (degrees/second)
    :param readout: readout time (seconds)
    :return:
    """
...
def get_order(
    params: dict,
    tile_struct: dict,
    tilesegmentlists, exposurelist, observatory, config_struct
):
    """
    Get the order of observations.

    :param params: dictionary of parameters
    :param tile_struct: dictionary of tile information
    :param tilesegmentlists: list of lists of segments for each tile
    :param exposurelist: list of segments for each exposure
    :param observatory: astropy.coordinates.EarthLocation
    :param config_struct: dictionary of configuration information
    :return: list of tile indices in the order of observation
    and list of filter indices in the order of observation
    """
...
def get_order_slew(params, tile_struct, tilesegmentlists, config_struct):
    """
    #FIXME: This function is never used. It is a copy of get_order,
    but missing some functions. Just delete.

    This function takes in the tile_struct and the tilesegmentlists
    and returns the order of the tiles to be observed.

    :param params: dictionary of parameters
    :param tile_struct: dictionary of tiles
    :param tilesegmentlists: list of tile segments
    :param config_struct: dictionary of configuration parameters
    :return: idxs, exposure_lists, filts
    """
    keys = tile_struct.keys()
    keynames = []
    namekeys = {}
    ...
    if params["scheduleType"] == "greedy_slew":
        current_time = Time(params["gpstime"], format="gps", scale="utc").mjd + np.min(
            params["Tobs"][::2]
        )
        ...
        while True:
            ...
            exptimecheck = np.where(
                current_time - lastObs < params["mindiff"] / 86400.0
            )[0]
            exptimecheckkeys = [keynames[x] for x in exptimecheck]
            idx2, slew_readout, exp_idle_seg = find_tile_greedy_slew(
                current_time,
                current_ra,
                current_dec,
                tilesegmentlists,
...
def scheduler(params, config_struct, tile_struct):
    """
    This function takes in the parameters, configuration, and tile structures

    :param params: dictionary of parameters
    :param config_struct: dictionary of configuration parameters
    :param tile_struct: dictionary of tiles
    :return: updated config_struct
    """
...
def computeSlewReadoutTime(config_struct, coverage_struct):
    """
    Compute the slew and readout time for the given configuration and coverage

    :param config_struct: configuration structure
    :param coverage_struct: coverage structure
    :return: accumulated slew and readout time
    """
    slew_rate = config_struct["slew_rate"]
    readout = config_struct["readout"]
...
def write_xml(xmlfile, map_struct, coverage_struct, config_struct):
    """
    Write the xml file for the given configuration, coverage and map
...
def summary(params, map_struct, coverage_struct, catalog_struct=None):
    """
    Function to produce summary plots and files

    :param params: parameter dictionary
    :param map_struct: map structure
    :param coverage_struct: coverage structure
    :param catalog_struct: catalog structure
    :return: None
    """
    ...
    for jj, telescope in enumerate(params["telescopes"]):
        ...
        schedulexmlfile = os.path.join(
            params["outputDir"], "schedule_%s.xml" % telescope
        )
        ...
        print(
            "Expected time spent on slewing and readout: "
            + str(slew_readout_time)
            + " s."
        )
...
def schedule_alternating(
    ...
    """
    Schedule observations in alternating filters.

    :param params: dictionary of parameters
    :param config_struct: dictionary of configuration parameters
    :param telescope: telescope name
    :param map_struct: dictionary of map parameters
    :param tile_struct: dictionary of tile parameters
    :param previous_coverage_struct: dictionary of previous coverage parameters
    """
    ...
    if (
        ...
        tile_struct_hold = gwemopt.utils.check_overlapping_tiles(
    ...
    for i in range(len(exposuretimes)):
        ...
        # if the duration of a single block is less than 30 min,
        # shift by additional time to add up to 30 min
        if i > 0:
        ...
        if params["doMaxTiles"]:
            tile_struct, doReschedule = gwemopt.utils.slice_number_tiles(
...
def schedule_ra_splits(
    ...
    """
    Schedule observations splitting the sky into RA slices.

    :param params: dictionary of parameters
    :param config_struct: dictionary of configuration parameters
    :param map_struct_hold: dictionary of map parameters
    :param tile_struct: dictionary of tile parameters
    :param telescope: telescope name
    :param previous_coverage_struct: dictionary of previous coverage parameters
    :return:
    ...
    while len(raslices) != 0:
        ...
        if len(coverage_struct["ipix"]) == 0:
            continue
        optimized_max, coverage_struct, tile_struct = gwemopt.utils.optimize_max_tiles(
            params,
            tile_struct,
            coverage_struct,
            config_struct,
        ...
        doReschedule, balanced_fields = gwemopt.utils.balance_tiles(
